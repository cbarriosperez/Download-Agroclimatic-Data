#####################################################################
## This script generates daily Tmax maps for the study-area extent ##
...
#################################################################################################
### This script section will make a for loop to extract daily maximum temperature from AgERA5 ###
#################################################################################################
...
##################################################################################################
## This script section will make a for loop to import the Tmax global daily maps and then clip  ##
## them using the Zoi shapefile. Next, a folder will be created for each year to save the daily ##
## maximum temperature maps of the study region.                                                ##
##################################################################################################
...
for(i in 1:(length(years))){
  ...
  rastlist = list.files(path = paste(getwd(),"/",1980+i,sep =""), pattern=paste("Temperature-Air-2m_Max-24h_C3S-glob-agric_AgERA5_",1980+i,sep =""), full.names = TRUE, recursive = FALSE)
  allrasters = terra::rast(rastlist)
  
  ##-- Crop and mask the Tmax maps to Zoi shapefile --##
  
  Tmax = crop(allrasters, Zoi)
  
  ##-- Convert Tmax maps from K to °C --##
  T.max.C = (Tmax - 273.15)

  ##-- resample the raster stack maps of Tmax --##
  Tmax.5km = resample(T.max.C,reference.5km, method = 'bilinear')
  res(Tmax.5km)==res(reference.5km)
  
  ##-- Create folders to save daily Tmax maps for each year --## 
  
  dir.create(paste("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/Tmax_5km_AgERA5/",1980+i,sep ="")) #change the folder
  
  for(j in 1:nlyr(Tmax.5km)){
    #j=1
    raster.name = time(Tmax.5km)[j]
    writeRaster(Tmax.5km[[j]],paste("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/Tmax_5km_AgERA5/",1980+i,"/Tmax_",raster.name,".tif",sep =""))
