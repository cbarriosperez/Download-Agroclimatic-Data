###########################################################################################################
## This script converts daily Tmin rasters for the study region into monthly averages.                   ##
...
setwd("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/Tmin_5km_AgERA5")

##########################
# Tmin-AgERA5.v2 - Daily #
##########################
...
rastlist.Tmin = list.dirs(getwd(), full.names = FALSE, recursive = FALSE)

# Number of years available
nyears = length(rastlist.Tmin)
...
for (i in 1:nyears){
  ...
  # Each file is assumed to be a single-day Tmin raster.
  ...
  for (j in 1:12) {
    ...
    # Average all daily Tmin layers for the current month to get monthly average
    Avg.Tmin = mean(allrasters[[ini:last]])
    
    # Zero-padded month string (e.g., "01", "02", ..., "12")
    month_str = sprintf("%02d", j)
    
    # Name the output raster layer (e.g., "Tmin_Avg_198101")
    names(Avg.Tmin)= paste0("Tmin_Avg_",1980+i, month_str) 
    
    # Write or export the average monthly Tmin raster to disk
    writeRaster(Avg.Tmin, paste0("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Mensual/Tmin_5km_AgERA5/Tmin_AgERA5-v2_", 1980+i, month_str, ".tif"))
