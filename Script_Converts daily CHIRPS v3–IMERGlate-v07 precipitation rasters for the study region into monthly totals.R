########################################################################################################
## This script converts daily CHIRPS v3-IMERGlate-v07 precipitation rasters for the study region into ##   
## monthly totals.The script loops through each year's folder, sums the daily TIFFs month-by-month,   ##
## and writes out a GeoTIFF for each monthly total with a consistent name.                            ##
...
setwd("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/CHIRPS_V3_IMERGlate-v07")

###################################
# CHIRPS-V3_IMERGlate-v07 - Daily #
###################################
...
for (i in 2:nyears){
  ...
  date = seq(as.Date(paste0(1999+i,'-01-01')), as.Date(paste0(1999+i,'-12-31')), by = "day")
  ...
  for (j in 1:12) {
    ...
    ini= which(year == 1999+i & month == month_start)[1]#Get the index of the first date corresponding to the year 'i' and the starting month
    last = which(year == 1999+i & month == month_end)[length(which(year == 1999+i & month == month_end))] 
    ...
    names(Total.rainfall)= paste0("Rain_Total_",1999+i, month_str) 
    
    # Write or export the monthly precipitation raster to disk
    writeRaster(Total.rainfall, paste0("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Mensual/CHIRPS_V3_IMERGlate-v07/Rain_Chirpsv3-IMERGlate_", 1999+i, month_str, ".tif"))
