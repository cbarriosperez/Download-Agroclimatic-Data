...
## using CHIRPS_V3_ERA5 data for 1981-2025.                                 ##
...
## and then clip them using the Colombia shapefile. Next, a folder will be created for each year to  ##
## save the daily precipitation maps of Colombia.                                                    ##
#######################################################################################################
...
for(i in 1:(length(years))){
  ...
  dir.create(paste("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/CHIRPS_V3_ERA5/",1980+i,sep ="")) #change the folder
  
  for(j in 1:nlyr(precipitation)){
    
    raster.name = names(precipitation)[j]
    writeRaster(precipitation[[j]],paste("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/CHIRPS_V3_ERA5/",1980+i,"/",raster.name,".tif",sep =""))
