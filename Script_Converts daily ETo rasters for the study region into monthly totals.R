###############################################################################################################
## This script converts daily ETo rasters for the study region into monthly totals.                          ##
...
setwd("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Diario/ETo_5km_AgERA5")
...
rastlist.ETo = list.dirs(getwd(), full.names = FALSE, recursive = FALSE)

# Number of years available
nyears = length(rastlist.ETo)
...
for (i in 1:nyears){
  ...
  # Each file is assumed to be a single-day ETo raster.
  ...
  for (j in 1:12) {
    ...
    # Sum all daily ETo layers for the current month to get monthly total
    Total.ETo = sum(allrasters[[ini:last]])
    
    # Zero-padded month string (e.g., "01", "02", ..., "12")
    month_str = sprintf("%02d", j)
    
    # Name the output raster layer (e.g., "ETo_Total_198101")
    names(Total.ETo)= paste0("ETo_Total_",1980+i, month_str) 
    
    # Write or export the monthly precipitation raster to disk
    writeRaster(Total.ETo, paste0("\\\\Catalogue/sat_pma/data/raw/Clima_Fuentes_Secundarias/Mensual/ETo_5km_AgERA5/ETo_AgERA5-v2_", 1980+i, month_str, ".tif"))
